# Direct-TMS Cross-Sectional Carry — Revised No-TRY Stage-A Preregistration

- **Status:** PREREGISTERED REVISION (2026-08-14). Locked on commit.
- **Prior specification:** `prereg/2026-08-08-tms-carry-kill-test-prereg.md` is retained unchanged
  with disposition **`SUPERSEDED_UNEXECUTED`**.
- **Reason:** explicit human universe/execution preference made before any Stage-A performance
  computation: no TRY exposure; `EUR_GBP` must be a genuine execution instrument.
...
- `prereg/2026-08-14-tms-carry-no-try-universe.json`
- `prereg/2026-08-14-tms-carry-no-try-mask.json`
- `prereg/2026-08-14-tms-carry-no-try-price-readiness.json`
...
`GBPUSD.pro` is not an execution leg. GBP exposure therefore genuinely uses `EURGBP` and is not
duplicated. Net all repeated `EURUSD` quantities before execution.

Each route is the zero-sum basis vector `e_c-e_USD`. Specifically,
`-(e_EUR-e_GBP)+(e_EUR-e_USD)=e_GBP-e_USD`. Thus for any `sum(w)=0` target, route quantity
...
`AUD_USD EUR_GBP EUR_HUF EUR_NOK EUR_PLN EUR_SEK EUR_USD EUR_ZAR NZD_USD USD_CAD USD_CHF USD_CZK USD_JPY`
